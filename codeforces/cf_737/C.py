...
for _ in range(*maps()):
    ...
    if n % 2:
        ...
        print(
            pow(cnt, k, mod)
        )  # all the choices are applied to each bit (there are total K bits)
    else:
        Pn = pow(2, n, mod)
        cnt = 0
        for i in range(0, n, 2):
        ...
        ans = pow(
            cnt, k, mod
        )  # for when there are no cases when all the kth bit of n elements are set
        for i in range(k):
            ans += (
                pow(cnt, i, mod) * 1 * pow(Pn, k - i - 1, mod)
            )  # 1 -- case where kth bit of all a_i's are set
            """
