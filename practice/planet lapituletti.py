	...
	for i in range(4):
		if s[i] not in d: 
			...
			if i <= 1 :
				...
				m+=d[s[i]]
	P = (h+m)[::-1] ; h = P[0] + P[1] ; m = P[2] + P[3]
	return s[0]+s[1] + ':'+s[2]+s[3] if 0 <= int(h) < H  and 0 <= int(m) <M else '-1' 

for _ in range(int(input())):
	h , m = maps() ; sh , sm = input().rstrip('\n').split(':') ; d = {'0' : '0' , '1' : '1' , '2':'5' , '5':'2' , '8':'8'}
	if check(sh+sm, d,h,m) != '-1':
		print(sh+':'+sm) ;continue
	c , H ,M ,mn1 , t1= 0 , int(sh) , int(sm) , None , [0, 0]
	while c < h+2:
		x = '' ; x , cnt = x +'0'+str(H) if len(str(H)) == 1 else str(H) , 0
		while cnt < m:
			y ='' ; y += '0' + str(M) if len(str(M)) == 1 else str(M)
			if check(x+y, d,h,m) != '-1':
				P = x+':'+y ; mn1, t1 = P , [c, cnt]
				break
			M = (M+1)%m ; cnt+=1
			if M ==0:
				break
