...
class Solution:
    def maximalNetworkRank(self, n: int, roads) -> int:
        ...
        for i in range(n):
            for j in range(n):
                if i != j :
                    ans = max(ans, (inn[i]+inn[j]) - ((i,j) in rdic))
