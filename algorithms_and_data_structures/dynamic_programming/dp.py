...
def ConvertRecur(str1 , str2 , m , n):
	...
	return 1 + min(ConvertStr1ToStr2(str1, str2, m, n-1 ) , ConvertStr1ToStr2(str1, str2, m-1, n) , ConvertStr1ToStr2(str1, str2, m-1, n-1))
	...
	for i in range(n+1):
		for j in range(m+1):
			if i== 0:
				...
				dp[i][j] = 1 + min(dp[i-1][j] , dp[i-1][j-1] , dp[i][j-1]) 	#Delete , Replace , Insert
	return dp[n][m]
...
def ConvertTopDownRecursiveDp(str1 , str2 , n , m , dp):
	...
	if str1[n-1] == str2[m-1]:
		dp[n][m] = ConvertTopDownRecursiveDp(str1, str2, n-1, m-1, dp) if dp[n-1][m-1] == -1 else dp[n-1][m-1]
		return dp[n][m]
	else:
		if dp[n-1][m-1] == -1:
...
n , m = 3 , 3
dp = [[0 for _ in range(m+1)] for __ in range(n+1)] ; dp[1][1] = 1
def TopDown(x , y ):
	if x==1 or y == 1:
		dp[x][y] = 1
		return dp[x][y]
	dp[x-1][y] , dp[x][y-1] = TopDown(x-1, y) if not dp[x-1][y] else dp[x-1][y] , TopDown(x, y-1) if not dp[x][y-1] else dp[x][y-1]
